...
| 13 | **キャリブレーション（2026-09-24 ユーザー指定）** | 計画 Opus 5.5／実装 Sol（task020 設定ファイルと RoomAnchor の設定化）∥ Sonnet（task021 キャリブレーション画面） | **実装中**。起動時に足元へ体験空間の枠（自分を中心、一辺 areaSize=1.6）と前方の扇形を表示、Quest 境界が取れれば緑/赤、取れなければ白。A/X 1秒長押しか運営 C で決定→ Ready。設定は persistentDataPath/play-area.json（事前に編集可）。登録は起動のたびに確かめ直す（Quest の座標基準が変わるため）。証拠は `--desktop --auto-calibrate --autostart`。ユーザーは後で Air Link で試す。実装済み（Sol: 設定と RoomAnchor オーバーロード、テスト19+12 PASS／Sonnet: 画面）。撮影で「枠が机に隠れる」と判明 → 追修正（キャリブレーション中は部屋を隠し無地の床にする）済み。追修正2（線が見えない→LineRenderer を View 向き・太さ2〜3cm、desktop の下向き60°）も済み。**キャリブレーション画面の証拠 evidence/20260924-calibration/calibration-screen.png（白い外枠・余裕の枠・水色の扇形・黄色の足元の印・案内）**。desktop の撮影はウィンドウが最小化されるため ShowWindow(9)＋SetForegroundWindow で戻してから PrintWindow。テスト19+12 PASS、ビルド0/0、例外0。独立レビュー: task020 Sonnet approve／Sol request_changes（CreateNew）、task021 両方 request_changes（赤で決定時の警告、凹形境界の判定、追跡不能時の部屋表示、C と開始の同一フレーム）→ 追修正を Sol（020）・Sonnet（021）に依頼中。**次: 追修正→ビルド・撮影→再レビュー→受入→commit** |
| 9 | 後の課題（非ブロッキング） | 計画: Opus 5.5 | (a) 生成後の `Model.Rules` 書き換え防止（Issue4 残り、task010 Sonnet 低）、(b) DemoRig の `inputs` 一覧を FloorReady と StartXR が共用（task012 Sonnet 低）、(c) MCP クライアントで一度だけ出た UnicodeDecodeError（review は正常保存。原因未特定）、(d) desktop の案内パネル文字のはみ出し（計画6・8、task015 で対応済み）、(e) `RoomVisuals.UpdatePublic` が `active` を無視し、暗転中も観客のアバターを表示・更新する（task019 の Sol 指摘、以前からの挙動）、(f) desktop 自動実行でプレイヤーが時々早期終了・最小化される（外部要因とみられる。証拠は最小化前のフレームを使う） |
